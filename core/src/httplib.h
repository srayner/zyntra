...
socket_t create_socket(const std::string &host, const std::string &ip, int port,
...
#ifdef _WIN32
            // Setting SO_REUSEADDR seems not to work well with AF_UNIX on
            // windows, so remove the option.
    ...
    for (auto rp = result; rp; rp = rp->ai_next) {
        ...
        if (rp->ai_family == AF_INET6) {
            set_socket_opt(sock, IPPROTO_IPV6, IPV6_V6ONLY,
                           ipv6_v6only ? 1 : 0);
        }
...
inline socket_t create_client_socket(
    ...
    auto sock = create_socket(
        ...
        [&](socket_t sock2, struct addrinfo &ai, bool &quit) -> bool {
            ...
            set_socket_opt_time(sock2, SOL_SOCKET, SO_RCVTIMEO,
...
inline bool zstd_compressor::compress(const char *data, size_t data_length,
    ...
    do {
        ...
        size_t const remaining =
            ZSTD_compressStream2(ctx_, &output, &input, mode);
            ...
            switch (state_) {
                ...
                case 2: {  // Headers
                    ...
                    while (pos < buf_size()) {
                        ...
                        constexpr const char header_content_type[] =
                            "Content-Type:";

                        if (start_with_case_ignore(header,
                                                   header_content_type)) {
                            file_.content_type = trim_copy(
                                header.substr(str_len(header_content_type)));
                        } else {
...
inline std::string random_string(size_t length) {
    ...
    thread_local auto engine([]() {
        ...
        std::seed_seq seed_sequence{seed_gen(), seed_gen(), seed_gen(),
                                    seed_gen()};
...
inline bool range_error(Request &req, Response &res) {
    if (!req.ranges.empty() && 200 <= res.status && res.status < 300) {
        ...
        for (auto &r : req.ranges) {
            ...
            // the size of the selected representation. If the last-pos value is
            // absent, or if the value is greater than or equal to the current
            // length of the representation data, the byte range is interpreted
            // as the remainder of the representation (i.e., the server replaces
            // the value of last-pos with a value that is one less than the
...
inline bool SocketStream::wait_readable() const {
    ...
    calc_actual_timeout(max_timeout_msec_, duration(), read_timeout_sec_,
                        read_timeout_usec_, read_timeout_sec,
                        read_timeout_usec);
...
inline bool ClientImpl::send_(Request &req, Response &res, Error &error) {
    {
        ...
        if (socket_.is_open()) {
...
#ifdef CPPHTTPLIB_OPENSSL_SUPPORT
            if (is_alive && is_ssl()) {
                if (detail::is_ssl_peer_could_be_closed(socket_.ssl,
                                                        socket_.sock)) {
                    is_alive = false;
                }
            }
            ...
            if (!is_alive) {
                // Attempt to avoid sigpipe by shutting down non-gracefully if
                // it seems like the other side has already closed the
                // connection Also, there cannot be any requests in flight from
                // other threads since we locked request_mutex_, so safe to
        ...
        if (!is_alive) {
            ...
            if (is_ssl()) {
                ...
                if (!proxy_host_.empty() && proxy_port_ != -1) {
                    auto success = false;
                    if (!scli.connect_with_proxy(socket_, req.start_time_, res,
                                                 success, error)) {
                        return success;
                    }
                }
...
inline bool ClientImpl::process_socket(
    ...
    return detail::process_client_socket(socket.sock, read_timeout_sec_,
                                         ...
                                         write_timeout_usec_, max_timeout_msec_,
...
inline void ssl_delete(std::mutex &ctx_mutex, SSL *ssl, socket_t sock,
    ...
    if (shutdown_gracefully) {
        ...
        // sent) and 1 on subsequent call (indicating close_notify alert
        // received)
